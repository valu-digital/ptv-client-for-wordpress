...
/**
 * PTV_Organization_In Class Doc Comment
 *
 * @category    Class
 * @description OPEN API V7 - View Model of organization for IN api
 * @author      Valu Digital
 * @link        https://www.valu.fi
 */
class PTV_Organization_In implements ArrayAccess {

	/**
	  * The original name of the model.
	  * @var string
	  */
	protected static $model_name = 'PTV_Organization_In';
	...
	public function list_invalid_properties() {
		...
		if ( null === $this->container['organization_type'] ) {
			$invalid_properties[] = "'organization_type' can't be null";
		}
		...
		if ( null === $this->container['organization_names'] ) {
			$invalid_properties[] = "'organization_names' can't be null";
		}
		if ( null === $this->container['display_name_type'] ) {
			$invalid_properties[] = "'display_name_type' can't be null";
		}
		if ( null === $this->container['organization_descriptions'] ) {
			$invalid_properties[] = "'organization_descriptions' can't be null";
		}
	...
	public function set_source_id( $source_id ) {
		...
		if ( ! is_null( $source_id ) && ( ! preg_match( '/^[A-Za-z0-9-.]*$/', $source_id ) ) ) {
			throw new InvalidArgumentException( "invalid value for $source_id when calling PTV_Organization_In., must conform to the pattern /^[A-Za-z0-9-.]*$/." );
		}
	...
	public function set_oid( $oid ) {
		if ( ! is_null( $oid ) && ( strlen( $oid ) > 100 ) ) {
			throw new InvalidArgumentException( 'invalid length for $oid when calling PTV_Organization_In., must be smaller than or equal to 100.' );
		}
		if ( ! is_null( $oid ) && ( ! preg_match( '/^[A-Za-z0-9.-]*$/', $oid ) ) ) {
			throw new InvalidArgumentException( "invalid value for $oid when calling PTV_Organization_In., must conform to the pattern /^[A-Za-z0-9.-]*$/." );
		}
	...
	public function set_municipality( $municipality ) {
		...
		if ( ! is_null( $municipality ) && ( ! preg_match( '/^[0-9]{1,3}$/', $municipality ) ) ) {
			throw new InvalidArgumentException( "invalid value for $municipality when calling PTV_Organization_In., must conform to the pattern /^[0-9]{1,3}$/." );
		}
	...
	public function set_business_code( $business_code ) {
		...
		if ( ! is_null( $business_code ) && ( ! preg_match( '/^[0-9]{7}-[0-9]{1}$/', $business_code ) ) ) {
			throw new InvalidArgumentException( "invalid value for $business_code when calling PTV_Organization_In., must conform to the pattern /^[0-9]{7}-[0-9]{1}$/." );
		}
	 ...
	 * @param string $publishing_status Publishing status (Draft or Published).
	 * @return $this
	 */
