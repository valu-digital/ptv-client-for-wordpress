...
/**
 * PTV_Phone_Channel Class Doc Comment
 *
 * @category    Class
 * @description OPEN API V7 - View Model of phone channel
 * @author      Valu Digital
 * @link        https://www.valu.fi
 */
class PTV_Phone_Channel implements ArrayAccess {

	/**
	  * The original name of the model.
	  * @var string
	  */
	protected static $model_name = 'PTV_Phone_Channel';
	...
	public function set_source_id( $source_id ) {
		...
		if ( ! is_null( $source_id ) && ( ! preg_match( '/^[A-Za-z0-9-.]*$/', $source_id ) ) ) {
			throw new InvalidArgumentException( "invalid value for $source_id when calling PTV_Phone_Channel., must conform to the pattern /^[A-Za-z0-9-.]*$/." );
		}
