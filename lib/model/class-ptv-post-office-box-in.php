...
/**
 * PTV_Post_Office_Box_In Class Doc Comment
 *
 * @category    Class
 ...
 * @link        https://www.valu.fi
 */
class PTV_Post_Office_Box_In implements ArrayAccess {

	/**
	  * The original name of the model.
	  * @var string
	  */
	protected static $model_name = 'PTV_Post_Office_Box_In';
	...
	public function set_postal_code( $postal_code ) {
		...
		if ( ( ! preg_match( '/\\d{5}?/', $postal_code ) ) ) {
			throw new InvalidArgumentException( "invalid value for $postal_code when calling PTV_Post_Office_Box_In., must conform to the pattern /\\d{5}?/." );
		}
	...
	public function set_municipality( $municipality ) {
		...
		if ( ! is_null( $municipality ) && ( ! preg_match( '/^[0-9]{1,3}$/', $municipality ) ) ) {
			throw new InvalidArgumentException( "invalid value for $municipality when calling PTV_Post_Office_Box_In., must conform to the pattern /^[0-9]{1,3}$/." );
		}
