 ...
 *
 * @category    Class
 * @description OPEN API V5 - View Model of Address
 * @author      Valu Digital
 * @link        https://www.valu.fi
 */
class PTV_Address_In implements ArrayAccess {
	...
	public function set_postal_code( $postal_code ) {
		...
		if ( ( ! preg_match( '/\\d{5}?/', $postal_code ) ) ) {
			throw new InvalidArgumentException( "invalid value for $postal_code when calling PTV_Address_In., must conform to the pattern /\\d{5}?/." );
		}
	...
	public function set_street_number( $street_number ) {
		if ( ! is_null( $street_number ) && ( strlen( $street_number ) > 30 ) ) {
			throw new InvalidArgumentException( 'invalid length for $street_number when calling PTV_Address_In., must be smaller than or equal to 30.' );
		}
	...
	public function set_municipality( $municipality ) {
		...
		if ( ! is_null( $municipality ) && ( ! preg_match( '/^[0-9]{1,3}$/', $municipality ) ) ) {
			throw new InvalidArgumentException( "invalid value for $municipality when calling PTV_Address_In., must conform to the pattern /^[0-9]{1,3}$/." );
		}
	...
	public function set_latitude( $latitude ) {
		...
		if ( ! is_null( $latitude ) && ( ! preg_match( '/^\\d+\\.?\\d*$/', $latitude ) ) ) {
			throw new InvalidArgumentException( "invalid value for $latitude when calling PTV_Address_In., must conform to the pattern /^\\d+\\.?\\d*$/." );
		}
	...
	public function set_longitude( $longitude ) {
		...
		if ( ! is_null( $longitude ) && ( ! preg_match( '/^\\d+\\.?\\d*$/', $longitude ) ) ) {
			throw new InvalidArgumentException( "invalid value for $longitude when calling PTV_Address_In., must conform to the pattern /^\\d+\\.?\\d*$/." );
		}
