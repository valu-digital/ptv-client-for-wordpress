...
/**
 * PTV_Street_Address_With_Coordinates Class Doc Comment
 *
 * @category    Class
 ...
 * @link        https://www.valu.fi
 */
class PTV_Street_Address_With_Coordinates implements ArrayAccess {

	/**
	  * The original name of the model.
	  * @var string
	  */
	protected static $model_name = 'PTV_Street_Address_With_Coordinates';
	...
	public function set_street_number( $street_number ) {
		if ( ! is_null( $street_number ) && ( strlen( $street_number ) > 30 ) ) {
			throw new InvalidArgumentException( 'invalid length for $street_number when calling PTV_Street_Address_With_Coordinates., must be smaller than or equal to 30.' );
		}
	...
	public function set_postal_code( $postal_code ) {
		...
		if ( ( ! preg_match( '/\\d{5}?/', $postal_code ) ) ) {
			throw new InvalidArgumentException( "invalid value for $postal_code when calling PTV_Street_Address_With_Coordinates., must conform to the pattern /\\d{5}?/." );
		}
	...
	public function set_latitude( $latitude ) {
		...
		if ( ! is_null( $latitude ) && ( ! preg_match( '/^\\d+\\.?\\d*$/', $latitude ) ) ) {
			throw new InvalidArgumentException( "invalid value for $latitude when calling PTV_Street_Address_With_Coordinates., must conform to the pattern /^\\d+\\.?\\d*$/." );
		}
	...
	public function set_longitude( $longitude ) {
		...
		if ( ! is_null( $longitude ) && ( ! preg_match( '/^\\d+\\.?\\d*$/', $longitude ) ) ) {
			throw new InvalidArgumentException( "invalid value for $longitude when calling PTV_Street_Address_With_Coordinates., must conform to the pattern /^\\d+\\.?\\d*$/." );
		}
