...
/**
 * PTV_Street_Address_In Class Doc Comment
 *
 * @category    Class
 ...
 * @link        https://www.valu.fi
 */
class PTV_Street_Address_In implements ArrayAccess {

	/**
	  * The original name of the model.
	  * @var string
	  */
	protected static $model_name = 'PTV_Street_Address_In';
	...
	public function set_street_number( $street_number ) {
		if ( ! is_null( $street_number ) && ( strlen( $street_number ) > 30 ) ) {
			throw new InvalidArgumentException( 'invalid length for $street_number when calling PTV_Street_Address_In., must be smaller than or equal to 30.' );
		}
	...
	public function set_postal_code( $postal_code ) {
		...
		if ( ( ! preg_match( '/\\d{5}?/', $postal_code ) ) ) {
			throw new InvalidArgumentException( "invalid value for $postal_code when calling PTV_Street_Address_In., must conform to the pattern /\\d{5}?/." );
		}
	...
	public function set_municipality( $municipality ) {
		...
		if ( ! is_null( $municipality ) && ( ! preg_match( '/^[0-9]{1,3}$/', $municipality ) ) ) {
			throw new InvalidArgumentException( "invalid value for $municipality when calling PTV_Street_Address_In., must conform to the pattern /^[0-9]{1,3}$/." );
		}
