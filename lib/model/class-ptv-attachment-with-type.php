...
/**
 * PTV_Attachment_With_Type Class Doc Comment
 *
 * @category    Class
 ...
 * @link        https://www.valu.fi
 */
class PTV_Attachment_With_Type implements ArrayAccess {

	/**
	  * The original name of the model.
	  * @var string
	  */
	protected static $model_name = 'PTV_Attachment_With_Type';
	...
	public function set_name( $name ) {
		if ( ! is_null( $name ) && ( strlen( $name ) > 100 ) ) {
			throw new InvalidArgumentException( 'invalid length for $name when calling PTV_Attachment_With_Type., must be smaller than or equal to 100.' );
		}
	...
	public function set_description( $description ) {
		if ( ! is_null( $description ) && ( strlen( $description ) > 150 ) ) {
			throw new InvalidArgumentException( 'invalid length for $description when calling PTV_Attachment_With_Type., must be smaller than or equal to 150.' );
		}
	...
	public function set_url( $url ) {
		if ( ( strlen( $url ) > 500 ) ) {
			throw new InvalidArgumentException( 'invalid length for $url when calling PTV_Attachment_With_Type., must be smaller than or equal to 500.' );
		}
