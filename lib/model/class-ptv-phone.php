 ...
 * @link        https://www.valu.fi
 */
class PTV_Phone implements ArrayAccess {

	/**
	  * The original name of the model.
	  * @var string
	  */
	protected static $model_name = 'PTV_Phone';
	...
	public function set_additional_information( $additional_information ) {
		if ( ! is_null( $additional_information ) && ( strlen( $additional_information ) > 150 ) ) {
			throw new InvalidArgumentException( 'invalid length for $additional_information when calling PTV_Phone., must be smaller than or equal to 150.' );
		}
	...
	public function set_charge_description( $charge_description ) {
		if ( ! is_null( $charge_description ) && ( strlen( $charge_description ) > 150 ) ) {
			throw new InvalidArgumentException( 'invalid length for $charge_description when calling PTV_Phone., must be smaller than or equal to 150.' );
		}
	...
	public function set_number( $number ) {
		...
		if ( ( ! preg_match( '/^\\d{1,20}$/', $number ) ) ) {
			throw new InvalidArgumentException( "invalid value for $number when calling PTV_Phone., must conform to the pattern /^\\d{1,20}$/." );
		}
